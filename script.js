...
function updateHeroImage() {
  ...
  const weapon = weapons[currentWeapon].name;

  if (weapon === "stick") heroImage.classList.add("hero-stick");
  else if (weapon === "dagger") heroImage.classList.add("hero-dagger");
  else if (weapon === "claw hammer") heroImage.classList.add("hero-hammer");
  else if (weapon === "sword") heroImage.classList.add("hero-sword");
}
...
function updateMonsterHealthBar() {
  const maxHealth = monsters[fighting].health;
  const healthPercentage = Math.max(0, (monsterHealth / maxHealth) * 100);
  monsterHealthBar.style.width = healthPercentage + "%";
}

function updateInventoryDisplay() {
  weaponsList.innerHTML = "";
  inventory.forEach((weaponName, index) => {
    const weaponData = weapons.find((w) => w.name === weaponName);
    const weaponDiv = document.createElement("div");
    weaponDiv.className = "weapon-item";
    if (index === inventory.length - 1) {
      weaponDiv.classList.add("active");
    }
    weaponDiv.innerHTML = `${weaponData.icon} ${weaponName}`;
...
function goFight() {
  ...
  const fightConfig = {
    0: { monster: "monster-slime", location: "cave" },
    1: { monster: "monster-beast", location: "cave" },
    2: { monster: "monster-dragon", location: "dragon" },
  };

  const config = fightConfig[fighting];
  monsterImage.classList.add(config.monster);
